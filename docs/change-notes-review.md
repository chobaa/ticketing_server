...
이 파일은 채팅에서 반영된 변경 사항과, 별도 커밋 없이 참고만 하면 되는 설명을 한곳에 모아 두었습니다. 필요하면 섹션 단위로 옮기거나 이슈/위키로 쪼개서 정리하시면 됩니다.
...
- [ ] 위 프론트/백엔드/스크립트 변경을 커밋 메시지와 함께 한 커밋 또는 논리 단위로 나눌지 결정
- [ ] `docs/`를 README나 팀 위키에서 링크할지 결정
- [ ] nGrinder 스크립트 쪽에 `progressMaxWaitSec`·좌석 순환을 `load-tests/ngrinder/README.md` 등에 문서화할지
